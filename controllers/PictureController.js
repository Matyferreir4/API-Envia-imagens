...
exports.deleteImage = async (req,res) => {
    try{
      ...
      const deleteImage = Picture.findbyIdAndDelete(picture)
      if (!deleteImage) {
        res.status(404).json({mensage: "Imagem não encontrada"});
      }
      res.status(200).json({mensage:"Imagem deletada com sucesso!"})
      } catch (error) {
        res.status(500).json({mensage: "Erro ao deletar a imagem!"})
      }
};
