...
function createPhotoCardElement(photo) {
  ...
  card.innerHTML = `
         ...
         <div class="exclusão">
            <buttom id="excluir">Excluir</buttom>
         </div>
         `;

  return card;

  const deleteButton = card.querySelector(`#excluir-${photo._id}`);
  if (deleteButton) {
    deleteButton.addEventListener('click', () => {
      // Chama a função de exclusão, passando o ID da foto
...
async function deletePhotos(photo) {

  // Monta URL para a imagem (API + ID da foto + /image)
  const imageUrl = `${config.apiUrl}/${photo._id}`;
  try {
    // Faz requisição GET para a API
    ...
    // Verifica se a resposta foi bem sucedida (Status 200-299)
    if (!response.ok) {
      throw new Error("Erro para deletar a imagem");
    }
    ...
    // Função de notificação sendo chamada para mostrar erro ao User
    showNotification("Falha ao deletar foto!", "error");
  }
}
